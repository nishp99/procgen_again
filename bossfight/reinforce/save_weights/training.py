...
parent_2 = os.path.join(parent, '..')
sys.path.append(os.path.join(parent_2, '..'))
#import procgen.ProcgenGym3Env
from procgen import ProcgenGym3Env


def train(weights_path, agent_health, policy, penalty, max_episodes, num_save_points, Nagents, lr, discount, beta): #,agent_health as additional parameter
	RIGHT = 7
	LEFT = 0
	...
	while total_episodes <= max_episodes:
		...
		if step > 0 and np.any(done):
			...
			#save weights to new file at every save point
			if any(save_points == total_episodes):
				true_model_path = os.path.join(weights_path, f'{total_episodes}.pt')
